export const validateEmail = (email: string) => /^[\w+.]+@\w+\.\w{2,}(?:\.\w{2})?$/.test(email.toString())
export const validateCustomerNameCaracteres = (fullName: string) => /.{5,100}/.test(fullName.toString())
export const validateCustomerName = (fullName: string) => /^[A-zÀ-ú ']+$/.test(fullName.toString())
export const validateCpf = (cpf: string) => /^\d{3}\.\d{3}\.\d{3}\-\d{2}$/.test(cpf.toString())
export const validateBirthDate = (birthDate: string) => /(\d{4})[-.\/](\d{2})[-.\/](\d{2})/.test(birthDate.toString())
export const validateProductName = (name: string) => /.{4,100}/.test(name.toString())
export const validateCategory = (category: string) =>  /.{4,50}/.test(category.toString())
